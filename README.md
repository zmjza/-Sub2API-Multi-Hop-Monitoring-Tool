...
- 单站、批量、GeeTest 和 Turnstile 添加成功后安全抓取同源 favicon 并持久化，失败不影响站点添加；历史缺失图标启动后受控补抓一次。
- 通知设置包括余额阈值、站点/渠道异常、恢复通知和冷却时间。
- 通用设置包括刷新频率、数据过期阈值、悬浮窗开关/位置和开机启动。
- 左侧“通知”进入通知规则，“设置”进入通用设置。

### Sub2API 服务器管理

- 左侧导航新增“Sub2API 服务器管理”，支持最多 50 个服务器的新增、查看、编辑、删除和打开。
- 每个服务器卡片最多展示 5 个同源快捷入口；快捷入口只在编辑弹窗内增删改排序，卡片上不显示删除。
- 服务器网页使用基于稳定 ID 的独立持久化 partition，重启后保留网页登录态；登录过期会提示“登录过期，请去站点管理重新验证”，支持一键清除登录状态。
- 内嵌网页保持 sandbox、contextIsolation、webSecurity 和同源导航白名单，不注入项目 preload，不导出 Cookie/Token。
...
- 渠道状态时间线固定 20 格，可用率按最近 12 次真实记录计算；余额只显示当前 Key 可信额度，不闪现站点总余额。
...
- `1.5.0` 优化在线更新组件：右上角版本徽标和设置页按钮使用同一套检查交互，显示加载/最新/失败反馈；发现新版本后自动进入“全部站点”并显示圆润更新卡片，包含版本对比、更新说明、跳过、稍后提醒和响应式操作区。
- `1.5.0` 保持 GitHub 稳定版为唯一安装包源。Windows 继续自动安装并重启；macOS 在免费 ad-hoc 条件下下载并打开 DMG，必要时手动替换 App。
...
- `1.4.8` 优化在线更新反馈：启动或点击版本徽标时显示检查状态，最新/失败结果显示消息，发现新版本时打开带更新说明和操作按钮的询问弹框。
- `1.4.9` 修复“稍后提醒”时间立即过期的问题，默认延后 24 小时再次检查。
- 1.4.6 及更早仍使用旧 Gitee 更新逻辑的客户端不能直接发现 GitHub Release，需要先安装已切换到 GitHub 更新源的版本。
