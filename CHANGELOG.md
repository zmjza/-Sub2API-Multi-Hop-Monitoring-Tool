...
- V2 渠道比例同时兼容 `0..1` 与 `1..100`，非法比例不再参与健康判断，避免健康渠道被错误标红。
- 使用记录首次进入和切换站点立即加载；“全部 API Key”不携带旧 Key，列表与统计独立容错，迟到响应不会覆盖当前查询。
- 顶部“平均耗时”改为“平均首字”；平均首字与平均缓存率分别按最近 30 个有效样本统计，并显示真实样本数。
- Sub2API 分组满 30 个有效样本且平均缓存率严格低于 80% 时，在 0 秒和 2 秒发送两次原生系统通知；持续异常与重启去重，恢复后重新布防。
- 缓存率告警复用现有刷新、`notification_states` 和系统通知能力，不新增设置控件、数据库表或第二套轮询。
- 更新 Vitest 与受影响间接依赖，官方 npm registry 审计为 0 个漏洞。

### 验证

- TypeScript、ESLint、定向与全量 Vitest、生产构建和 Electron E2E 通过。
- macOS 源程序真实使用记录首次加载、最近 30 次统计和 V2 渠道页面已复核；原生通知节奏使用受控数据验证。
- Windows 仅执行 x64 NSIS 交叉构建和结构验证，不代表 Windows 真机通知验收。
- macOS ARM64 DMG：101894445 字节，SHA-256 `02ae0d6470845c6a7b320c86f54e0ef6b83035205cf007ff3671ca80c567a310`。
- Windows x64 NSIS：94552146 字节，SHA-256 `4f95d5e6026965082c23da5ac06e97642d04a7559d5ee040b5dc31b3fff5613b`。Windows 仅为交叉构建和结构验证。

## 3.4.0 - 2026-09-19

### 禾维 AI 自动填入修复与界面优化

- 适配禾维 AI 当前两阶段 API Key 控件：先用受控可信点击激活输入框，再通过原生文本输入更新 React 状态，接口地址与 Key 均可稳定自动填入。
- 自动关闭禾维地区说明，并将“接口配置”滚动到首屏；刷新页面后会重新自动填入，仍不自动点击“开始检测”。
- 禾维工具栏改为检测中心标题、当前站点、填入状态徽标及带文字的重选、刷新、关闭按钮，操作状态更清晰。
- 自动填入结果只返回布尔状态和安全坐标，不向 Renderer、日志或文档暴露完整 API Key。

### 验证

- macOS 源程序真实点击验证：奇美拉地址自动填入为 baseUrl/v1，Key 由禾维页面脱敏显示；刷新后自动重填，检测按钮未触发。
- Vitest：63 个测试文件、489 项通过；TypeScript、ESLint、格式和生产构建通过。
- Electron E2E：14 项通过、2 项按既有真实站点条件跳过。
- macOS ARM64 DMG 通过 hdiutil 与 codesign 校验，包内版本为 3.4.0；SHA-256：`5a71ba197d1cc409f3b7d32432c8df85fd510110350049a48007774d9c376337`。
- Windows x64 NSIS 为有效安装器 PE，包内应用为 x86-64 PE、asar 版本为 3.4.0；SHA-256：`a3d545807efddd57406e9868ee70b20eab5698082e25a277e9a173f5c6c4af5b`。
- Windows 仅为 macOS 上的 x64 交叉构建和结构验证，不代表 Windows 真机验收。

## 3.3.0 - 2026-09-20

### API 密钥、禾维 AI 与使用记录优化

- API 密钥页填满主窗口剩余高度，左侧渠道卡独立滚动，不再随 Key 数量收缩。
- 分组下拉菜单挂载到页面顶层，并根据视口空间向上或向下展开，避免被表格裁切。
- 全部站点新增“智商检测中心”：每次先手动选择站点和当前 Key，再在受控禾维 AI 内嵌页自动填入站点模型接口地址（baseUrl/v1）与 API Key；不设置默认站点，不自动开始检测。
- 禾维 AI 完整 Key 仅由主进程读取并填入固定 HTTPS 页面，不进入 Renderer、URL、日志或持久化配置。
- 悬浮窗“今日 Token / 今日消费”数字由 24px 缩小为 20px。
- 使用记录选择“全部 API Key”时，列表不再因统计接口单独失败而被清空；空白 Key 筛选统一归一化为未指定。

### 验证

- Vitest：63 个测试文件、487 项通过。
- TypeScript、ESLint、生产构建通过。
- Electron E2E：14 项通过、2 项跳过。
- 禾维真实页面自动填充与 macOS 打包应用视觉仍以本轮真机记录为准；Windows 只做 x64 交叉构建证据，不代表 Windows 真机通过。
- macOS ARM64 DMG SHA-256：`780f5dadafd63f89992d9cb57759d3c2642bc7c61f7f14442fe6be8fd5dd80cd`。
- Windows x64 NSIS SHA-256：`b9d21a017bebdbae9d59d1d2401c0d5b9075e26242aaeb92208a5577238cc588`。

## 3.2.0 - 2026-09-17

### 悬浮窗真正铺满 380×260

- 悬浮窗卡片改为 `width/height: 100%`，跟随原生 380×260 窗口，不再在窗口里再套一层 260px 固定盒。
- 今日 Token / 今日消费用 `grid-template-rows: 1fr` 撑满渠道行到页脚的剩余高度，数字加大到 24px 并垂直居中。
- 源码 Electron 窗口实测：指标区约 74px，按钮高度与指标区一致，渠道行到指标、指标到页脚间隙为 0。

### 验证

- 源码启动 Electron，`innerHeight=260`，`metrics.height≈74`，`btnFill=0`。
...
- 380×260 悬浮窗改为纵向 flex：渠道卡回到文档流，今日 Token / 今日消费吃掉渠道行到页脚之间的剩余高度，不再绝对贴底留白。
- 页脚保持 `box-sizing: border-box` 固定 49px，不再绝对定位；指标区与页脚不相交。
- 双击今日 Token / 今日消费进入使用记录的行为不变。
...
- V2 矩阵桶仅返回成功率/错误率而没有请求计数时，正确识别为有流量并按可用率与首 Token 着色。
- 每次渠道完整刷新重新先探测 V1，避免缓存的 V2 协议和临时 403 让页面长期进入灰色状态。
- V2 时间桶数量按 90m、24h、7d、30d 当前窗口计算。

### 验证

- Vitest：60 个测试文件、453 项通过。
- typecheck、ESLint、Prettier、生产构建通过。
- Electron E2E：12 项通过、2 项跳过；1 项既有窗口工作区几何断言受当前显示器环境影响失败。

## 2.11.0 - 2026-09-14

### 渠道状态与连通性测试

- 增加 V2 snapshot/matrix 载荷归一化，保留覆盖、回填、无流量和未来桶语义。
- 渠道状态页、总览渠道摘要和渠道状态弹窗按 `monitorSource` 分别展示 V1/V2 指标，避免协议串壳。
- 测试连通性按站点与 Key 请求 `/v1/models`，兼容数组、`data` 和 `models` 响应，并自动选择首个模型。
- 使用记录继续按最近 100 条有效记录计算平均耗时和平均缓存率。

### 验证

- Vitest：60 个测试文件、449 项通过。
- typecheck、ESLint、Prettier 和生产构建通过。
- Electron E2E：本地 Sub2API 全流程用例通过；完整 15 项运行中 11 项通过、2 项跳过、2 项为既有窗口几何/外部站点条件问题。
- Windows 仅保留交叉构建边界；macOS ARM64 真机页面截图与正式双平台 Release 待执行。

## 2.10.0 - 2026-09-13

### 修复渠道探测与测试连通性

- 渠道状态探测改为先请求 V1 `/api/v1/channel-monitors`，仅在 V1 不支持或返回空数据时探测 V2 矩阵接口；返回载荷保留 `monitorSource`，全部站点、渠道页和状态弹窗可按来源复用。
- 测试连通性弹窗改为按所选站点和 Key 请求站点根地址 `/v1/models`，自动选中该 Key 返回的第一个模型；切换 Key 会清空并重新加载模型，避免旧模型误用于新 Key。
- 模型列表兼容数组、`data` 和 `models` 三种常见响应形状，并在 IPC 边界校验站点与 Key。

### 验证

- Vitest：60 个测试文件、448 项通过。
- typecheck、ESLint、Prettier、Renderer/Electron 生产构建通过。

## 2.9.0 - 2026-09-13

### 测试连通性

- 全部站点卡片在“查看渠道状态”右侧新增“测试连通性”。
- 弹窗只做普通文本：先选当前站点 active Key（默认当前生效 Key），再选模型（默认第一个可用模型），提示词默认 hi。
- 主进程读取完整 Key 并发起一次真实请求；弹窗标注会产生真实用量。结果只留在本次弹窗，不写回站点卡片。
- 支持流式输出、取消、超时、重试和单飞；完整 Key 不进入 Renderer、IPC 返回、日志或截图。

### 使用记录统计

- 平均耗时和平均缓存率改为当前筛选结果按时间倒序最近 100 条有效样本；总请求数、总 Token、总消费仍用筛选全量。
- 新增平均缓存率卡片，统计区现为五张：宽屏单行，窄屏两列，空样本显示 —。中转站与 OpenCodex 同一口径。

### 渠道状态 V2 兼容

- 先探测 GET /api/v1/channel-monitor-v2/matrix?range=90m&group_by=platform_group；仅 404/405/未启用/契约非法时回退旧版。
- 401/403 显示鉴权错误，不伪装成旧版成功；429/5xx/超时保留缓存并下周期重探 V2。
- healthy/warning/critical/unknown 映射为现有 normal/degraded/failed/unknown；无流量显示未知。不改渠道 UI 结构。

### 验证

- 定向单测、IPC/适配集成、lint、typecheck、format 和完整 Electron E2E 通过。
- macOS 真机与页面样式验收通过；Windows 仅 x64 交叉构建和结构验证。

## 2.8.0 - 2026-09-04

### OpenCodex 全量历史记录

- 不再依赖 `/api/logs` 最近 2000 条内存环形缓冲。
- 改用 `/api/request-history` 的 `from/to`、provider、model、status 服务端筛选和 cursor 分页，自动拉取当前范围全部匹配记录。
- 使用记录来源文案改为“当前范围全部记录”，筛选范围切换时重新请求对应完整数据。
- 保留本地 Renderer 的思考模式、请求类型、排序、分页和列设置能力。

### 验证

- 本机真实 OpenCodex 今日范围返回 4677 条，已完整拉取并汇总，不再截断为 2000 条。
- Vitest（56 个测试文件、422 项）、typecheck、lint、build 和 OpenCodex 分页回归通过。

## 2.7.2 - 2026-09-04

### 彻底修复内嵌网页拦截主导航

- 关闭 Sub2API 服务器、Radar、常用网站内嵌页时，Renderer 立即清空嵌入态，不再等待异步销毁回调。
- 保留主进程当前 WebContentsView 身份校验，旧网页事件无法覆盖新的主导航 Shell。
- 新增“内嵌页 → 全部站点”回归用例，覆盖主导航切换后的嵌入态清理。

## 2.7.1 - 2026-09-04

### 修复内嵌网页拦截主导航

- 修复关闭 Sub2API 服务器或常用网站内嵌页后，旧网页排队事件回写 `open` 状态，导致“全部站点”等主导航点击无效的问题。
- 服务器和常用网站状态同步现在校验当前 WebContentsView 身份，旧视图事件不会覆盖当前 Shell。

### 验证

- 常用网站内嵌页切回“全部站点”回归用例通过。
- 其余自动化和构建结果沿用 2.7.0 基线；本修复需重新生成并发布 2.7.1 双平台资产。

## 2.7.0 - 2026-09-04

### OpenCodex 与嵌入式导航优化

- 删除总览“渠道推荐”展示区，保留倍率查看、充值比例、渠道状态和手动关联能力。
- OpenCodex 日志默认加载上限提升至 4000 条；真实输入按 `inputTokens - cacheReadInputTokens` 计算，缓存读取/写入、总 Token、首字、耗时和 t/s 使用服务端字段，并新增 Token 右侧缓存率列。
- 今天、近 7 天、近 30 天改为本地自然日边界，自定义范围继续包含完整日期端点。
- Sub2API 服务器、Radar、常用网站均支持模块内顶部胶囊快速切换；支持连续点击请求序列，新页面加载成功后再替换旧页面，失败时恢复旧页面，减少切换空白和卡顿。
- 现有页面补充统一页面进入、导航 hover/focus 和 reduced-motion 动效。

### 验证

- `npm test -- --run`：56 个测试文件、420 项通过。
- `npm run typecheck`、`npm run lint`、`npm run build` 通过。
- Electron E2E：完整套件 12 项通过、2 项跳过；OpenCodex 几何、站点主流程和常用网站流程通过。

## 2.6.0 - 2026-08-29

### 使用记录与渠道界面优化

- 删除全部站点页面中“渠道推荐随渠道状态全局同步刷新（10–20 秒）”的展示文案，保留后台统一刷新能力。
- 中转站使用记录将缓存率从 Token 单元格拆出为独立列；Token 单元格仅保留输入、输出和缓存读取三行。
- OpenCodex Token 单元格删除缓存率显示，继续独立展示输入、输出和缓存读取。
- 修复主进程渠道广播只更新共享 loader、未必触发全部站点卡片重渲染的问题；总览卡片现在按站点同步最新渠道缓存。
- 修复已打开 Sub2API 服务器时无法点击其他服务器切换的问题，并在切换失败时尽量恢复原页面。
- 全部站点卡片和悬浮窗的关联渠道增加渠道状态最后更新时间，统一显示本地 `HH:mm:ss`。
- 修复 Sub2API 原生页面打开后仍停留在全部站点 Shell 导致内容视觉叠加的问题；打开服务器时自动进入服务器 Shell，切回其他菜单会关闭原生视图。

### 验证与限制

- 本轮新增定向测试覆盖缓存率独立列、OpenCodex 无缓存率、服务器切换状态和总览渠道广播接线。
...
- 主进程新增独立渠道刷新调度，随机 10–20 秒运行，窗口隐藏、最小化或切换悬浮窗时继续执行；渠道结果带 `fetchedAt`/stale 状态并通过 IPC 广播，页面不再维护渠道后台定时器。
- 全局站点刷新改为随机 30–60 秒自动调度；手动全局刷新同时触发站点、渠道和倍率数据。
- 使用记录与 OpenCodex 使用记录新增共享缓存率计算与 Badge，颜色区间为 0–30% 红、31–60% 黄、61–85% 绿、86–100% 紫；OpenCodex 缓存读取补充图标。
- Radar 顶层导航允许安全 HTTPS 跨页面跳转；Sub2API 内嵌网页工具栏增加服务器快速切换栏。

### 验证与限制

- `npm test -- --run`：54 个测试文件、411 项通过；`npm run typecheck`、`npm run lint`、`npm run build` 通过。
- 本版本的真实 macOS 打包应用、真实站点刷新等待、页面视觉和发布资产尚未在本轮完成，待继续验收；Windows 仍只做交叉构建证据。

## 2.5.1 - 2026-08-19

### 修复在线更新下载失败

- 下载遇到 GitHub CDN 瞬时断连、连接重置或 5xx 响应时自动重试，重试前清理不完整临时包；下载流仍会进行 SHA-256 校验，避免损坏安装包进入安装流程。
- 新增更新服务回归测试，覆盖瞬时网络失败重试和断流清理。

## 2.5.0 - 2026-08-19

### 渠道关联改为仅手动指定

- 全部站点的站点状态卡片与悬浮窗不再把自动渠道解析结果当作最终展示来源：用户未手动关联时显示“暂未关联渠道”，不再根据 Key.group_id、分组名称、平台、模型或 /channels/available 关系自动补出渠道。
- 共享解析器调整为“手动关联优先，自动候选仅作倍率推荐内部带状态候选池”；总览卡片、悬浮窗与渠道状态弹层的最终关联来源统一为手动保存的 channelIds。
- 用户清除手动关联后，站点卡片、悬浮窗和弹层立即回到“暂未关联渠道”，不残留旧自动匹配结果或旧主渠道；多渠道、站点隔离、保存/清除反馈、刷新、stale-while-revalidate 和局部错误隔离保持不变。
- 卡片渠道摘要的来源标签统一为“手动指定”；倍率推荐沿用“手动优先、自动候选兜底”的既有评分逻辑，不对其他推荐规则做无关改写。
...
- 模式切换按钮文案随模式变化：中转站模式显示“切换 opencodex 模式”，OpenCodex 模式显示“切回中转站模式”；位置、选中态与点击行为保持不变。
- OpenCodex 请求记录列拆分：原“提供方模型”单列拆为“提供方”与“提供方模型”两列，“状态”列前移至第三列；最终列顺序为时间、提供方、提供方模型、状态、思考模式、请求类型、Token、首字、耗时 / t-s、实际消费，列设置面板同步。
- 思考模式统一英文显示：OpenCodex 与中转站模式均显示 low / medium / high / xhigh / max（缺失为占位符），筛选下拉选项同步为英文。
- 耗时 / t-s 徽标分级配色与中转站完全一致：小于 20 t/s 慢（橙红）、20–50 正常（蓝）、大于等于 50 快（绿）、无法计算为灰色不可用；复用现有 usage-speed-badge 规则。
...
- 使用记录页在"当前选中中转站"旁新增"切换 opencodex 模式"按钮，可切换到本机 OpenCodex 日志视图并随时切回，中转站模式功能、筛选、统计、分页与导出保持不变。
- OpenCodex 数据源固定为 http://localhost:10100/api/logs，主进程从 ~/.opencodex/admin-api-token 读取管理员令牌以 Bearer 请求；令牌只在主进程内存使用，不进入 Renderer、日志或持久化。
- 请求记录展示：时间、提供方模型、思考模式、请求类型、Token（输入/输出/缓存）、首字、耗时 / t-s、实际消费、状态；字段缺失显示占位符，实际消费使用接口估算值，不伪造数据。
- 筛选支持今天/近 7 天/近 30 天/自定义日期、提供方、模型、思考模式、请求类型、状态、最新/最早排序、刷新、列设置和分页；最近 2000 条日志在前端完成筛选与统计。
- 错误态覆盖令牌缺失、401/403、服务未启动、超时、非 JSON 和结构异常，失败不阻塞切回中转站模式。
- 新增共享 Zod 契约、主进程 opencodex-service、IPC opencodex:logs、preload 桥和 Renderer 独立组件，不修改 OpenCodex 服务端，不写入 Sub2API 数据库。

### 验证

- 新增契约、服务、归一化/筛选/统计单测；全量 Vitest 53 文件 403 项通过，ESLint、Prettier、TypeScript 和生产构建通过。
...
- 2.2.0 发布过程中第一次构建上传了 DMG，第二次构建重建 DMG 后因资产名已存在被发布脚本跳过上传，但 manifest 使用新构建哈希，导致线上 DMG 与 manifest SHA-256 不一致，客户端下载后校验失败并提示“更新下载失败，请稍后重试”。
- 修复发布脚本：非复用产物模式下先删除同名已有资产再上传，保证 DMG、EXE、blockmap 与 manifest 来自同一次构建；新增 assetsToReplace 单测。
- 2.2.1 使用同一次构建生成并上传全部资产，客户端检查更新可正常发现并下载 2.2.1。
...
- DMG `cacfcf0bf65d5974ae0ee824f820ced30ceedc85b57e1cf713b641a189ff7939`；EXE `11e583581bbc1e0311a175a3f0408fc475dcecef5ca88e388ec006c68bd11072`；blockmap 与 `update-manifest.json` 已随 Release 上传，远端 manifest 哈希 `35fdf06c53abec12775f9d114d5df5d7060212f6e7d39f7d2523b354318dfac1`。

## 2.1.0 - 2026-08-09

### Sub2API 快捷入口菜单化

- 新增服务器弹窗不再要求手填快捷入口名称、路径、图标或排序，只保留名称、HTTPS 根地址和可选登录页识别规则；保存并打开服务器完成登录后，编辑弹窗从服务器真实导航菜单中选择快捷入口。
- 编辑弹窗新增“快捷入口（已选 n/5）”菜单选择器：直接列出服务器当前账号可见的同源 HTTPS 菜单，勾选即加入、取消即移除，达到 5 个上限后未选项禁用；顺序默认跟随服务器菜单顺序。
- 菜单发现由主进程执行：优先复用已打开的同服务器持久化分区视图，否则创建同 partition 临时隐藏视图，加载首页后读取 `nav`、`aside`、`role=navigation` 等语义导航和 SPA 链接，完成后立即摘除；登录、注册、退出、外链、空链和危险协议不参与候选。
- 菜单名称、路径和上级菜单名按服务器隔离缓存到 SQLite，图标由应用本地按名称/路径映射为 Lucide 图标；获取菜单时显示加载、未登录、登录过期、错误和无菜单状态，登录过期提示重新登录后刷新，失败保留重试。
- 旧版手工快捷入口自动兼容：能按同源规范化路径匹配新菜单时显示为已选，匹配不到的显示在“当前菜单不可用”区域，可单独移除，未移除并保存时继续保留。
- 清除登录状态、更换服务器 origin 或删除服务器时同步清理对应菜单缓存；菜单发现不读取 Cookie、Token、页面存储、完整 HTML 或私有接口响应，不把远程脚本或页面数据传回 Renderer。

### 验证与发布

- 新增共享菜单 schema、同源净化、去重/登录过滤、按服务器菜单缓存和旧 shortcut `menuId` 兼容测试；Electron E2E 覆盖新增无手填快捷入口、旧入口兼容和“获取菜单”入口。
- 格式检查、ESLint、TypeScript、全量 Vitest（48 文件、366 项）、生产构建、Electron E2E（9 项通过、1 项按配置跳过）与官方源 `npm audit` 0 漏洞通过；macOS ARM64 打包应用在本机真实 HTTPS 夹具完成菜单发现、选择 5 个、保存与重启保留，证据目录 `real-test-evidence/macos-2.1.0/`。
- api-feng.online 完整菜单登录受站点 Turnstile 与前端鉴权守卫限制，应用内嵌与登录状态边界已验证，但真实菜单列表不冒充完整通过。
- DMG `6f1ada34aad2ffa1f290b6775728f0ad65ba8f7707a963a42f364aabc9250630`；DMG blockmap `788166ebd0ee5878b5d32fa3822c6fdd9115a379fbff154d4eb0e242ddae7b47`；EXE `f7ee5c144f9adce25d57bb6e219b44f0ea8e127d3569ba5675feaa6701276654`；EXE blockmap `f4b03ee32324ce45e7e6c501bf45e4b1f350e5235bef46ae2846aae1eb04593e`。

## 2.0.0 - 2026-08-09

### Sub2API 服务器管理

- 左侧导航在“站点管理”与“雷达”之间新增“Sub2API 服务器管理”，支持最多 50 个服务器的新增、查看、编辑、删除和打开；数据保存在 SQLite `sub2api-servers:entries`，名称与规范化 HTTPS 地址不可重复。
- 卡片展示名称、域名、登录状态和最多 5 个同源快捷入口；快捷入口在卡片上不显示删除，只在编辑弹窗内新增、修改、删除和排序，并支持账号管理、使用记录、API 密钥、渠道管理、系统设置等 Lucide 预设图标。
- 新增、编辑、删除弹窗支持行内校验、提交防重、Esc、遮罩关闭、焦点陷阱、关闭后焦点恢复和长文本防溢出；删除会清理当前视图与对应服务器的 Cookie、LocalStorage、IndexedDB、Cache Storage 和缓存。
- 服务器网页使用基于稳定 ID 的独立 `persist:` partition，重启后保留网页登录态，服务器之间完全隔离；内嵌工具栏提供返回、前进、主页、刷新、关闭，主窗口 resize 同步 bounds。
- 登录状态按“状态未知 / 请登录 / 已登录 / 登录过期”展示；首次打开登录页显示“请登录”，曾登录后被同源登录路由或主页面 401/403 带回时显示“登录过期”并提醒“登录过期，请去站点管理重新验证”；提供清除登录状态并重新登录。
- 远程视图保持 sandbox、contextIsolation、nodeIntegration=false、webSecurity=true、不注入项目 preload、不开放业务 IPC，并拒绝额外 webview、危险协议和跨域顶层导航。

### 站点 favicon 与现有问题修复

- 单个密码添加、批量添加、GeeTest 和 Turnstile 交互添加成功后统一安全抓取同源 favicon；失败不阻塞站点添加，批量成功项即时回填任务卡片，历史缺失站点启动后受控补抓一次。
- 全部站点“无渠道状态”最低价推荐按 NFKC 规范化后的 API/分组名称过滤“生图、停用、禁止、image2、图片”字段，不误删真实渠道候选；全部被过滤时显示无合格推荐。
- 渠道状态时间线过滤无效及未来时间，按 checkedAt 旧到新排列并只保留可完整显示记录，最右格为最新状态；最近一分钟存在 normal/degraded 可靠记录时不再被旧顶层失败误判为异常。
- 悬浮窗渠道状态由 12 格改为固定 20 格，记录不足时左侧补空；文案保持“近 12 次可用 xx%”，百分比只按最新 12 条真实有效记录计算，补空不进分母。
- 悬浮窗余额只展示当前 Key 可信额度，Key 未解析、切换中、加载失败或无可信值时显示 `—`，禁止回退并闪现站点总余额，也禁止旧 Key 数值串到新 Key。
- 自动或手动刷新遇到认证失效时统一提示“登录过期，请去站点管理重新验证”，使用稳定通知 ID 防堆叠，重新验证成功后清除过期展示。
- 调转菜单映射：“通知”使用铃铛进入通知规则页面，“设置”使用设置图标进入通用设置页面；同步修正 E2E、截图名称和旧文档中的相反描述。
