...
同一 V2 中转站的官方渠道页显示健康，桌面端却把部分分组标成异常；缓存率和首 Token 看起来合理，可用率明显偏低。

**根因**

V2 上游的 `success_rate`、`error_rate` 和 `cache_rate` 可能返回 `0..1` 小数，也可能直接返回 `1..100` 百分数。适配器原先统一按小数乘 100，导致 `98.4` 被放大后参与错误的健康判断。

**正确做法**

比例字段统一走同一归一化入口：`0..1` 保持小数语义，`1..100` 除以 100；负数、超过 100、缺失和非有限值不参与健康阈值。成功率、错误率和缓存率必须使用同一规则，禁止按站点、域名或分组特判。

**验证方式**

运行 `electron/main/adapters/channel-monitor-v2.test.ts`，覆盖 `0.984`、`98.4`、0、100、负数、超过 100 和非有限值；再在真实 V2 站点确认渠道卡不再一律红色。本轮真实桌面页面已显示健康、需关注和异常的差异状态；官方页面同凭据完整网络字段对照信息不全，待人工补充。

**禁止事项**

不要对所有比例无条件乘 100；不要让非法值进入健康判断；不要为单个中转站写硬编码修复。

**相关文件或命令**

- `electron/main/adapters/channel-monitor-v2.ts`
- `electron/main/adapters/channel-monitor-v2.test.ts`

**适用范围**

Sub2API channel-monitor-v2 的 snapshot、matrix 和以后新增的比例字段。
