...
- 在 `.site-card-actions` “查看渠道状态”右侧增加“测试连通性”，沿用现有按钮视觉和四按钮单行网格；宽屏与 720px 单列均不得换行或溢出。
- 弹窗复用现有弹层：站点信息、active Key 下拉、模型下拉、提示词（默认 hi）、深色输出区、关闭/开始测试；结果仅会话内保留。
- Renderer 只传 siteId/keyId/model/prompt；完整 Key 只在主进程读取。

## 2026-07-29 全局通知与渠道持续展示增量（待接入）

- 主框架新增唯一通知 Viewport，固定右上且位于标题栏下方，桌面宽约 360px、窄窗左右各 16px，最多三条；不得挤压主内容、遮挡窗口控制或抢焦点。
- 通知五态使用 Lucide 图标与绿/琥珀/红/中性色点缀，8px 圆角、白色高不透明表面和克制阴影；loading 原位更新为结果，长文案换行但操作按钮不溢出。
- 总览渠道摘要刷新时继续展示最近成功数据和全部手动关联；失败附加“更新失败，显示上次数据”、最后成功时间与图标重试入口，卡片高度和 footer 基线不变化。
- 首次无缓存失败才使用完整错误槽；auth-required/unsupported 保持独立语义。后台自动轮询失败不进入全局通知队列。
- 视觉验收必须覆盖成功、刷新、失败保留、自动恢复、多渠道、长名称、宽窗和约720px；验证通知与渠道摘要互不遮挡。状态：`待组件/E2E/真实 Electron 页面检查`。
...
| ------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| `src/renderer/shells/overview/OverviewPage.tsx`                                                                     | 替换顶部三指标和卡片 Key 指标；关联渠道区域消费九态；总览歧义候选内择优后，卡片与详情请求共用最终渠道 ID            | current-key snapshot、跨站 aggregate、channel relation；loading/partial/stale/unknown/unsupported | Key 变化和快速切站丢弃旧结果；未确定不显示整站值；订阅元数据不覆盖金额 |
