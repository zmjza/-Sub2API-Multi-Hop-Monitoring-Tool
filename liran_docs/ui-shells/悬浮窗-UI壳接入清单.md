...
- 右侧渠道卡只保留参考截图红框中的可用率文字和 12 个状态格，删除来源、渠道名和状态徽标可见行；卡片高度同步收缩，不保留空白占位。
- 时间线改为服务端 `timeline` 最近 12 次真实检测：左旧右新、最右最新；不足 12 条时左侧补浅灰空槽，零条显示“暂无渠道记录”，未知状态使用更深灰色；可用率文案统一为“近期可用率 xx%”。
- 真实状态格展示本地化检查时间和中文状态 tooltip/ARIA；按钮访问名称保留主渠道名并继续打开全部关联渠道，弹框同步显示“近期可用率”。
- 固定 `380×260`、主渠道选择、30–60 秒随机刷新、手动全刷新、t/s、余额、统计、footer 和弹框焦点/关闭行为不变；1.9.2 DMG 挂载态满历史/弹框与同源打包目录短历史截图均已检查通过。

## 1.9.1 已完成增量

- t/s 徽标从主体右上角移动到底栏空白区，与运行状态、扩大和刷新操作同排，保留 Gauge、两位小数和慢/正常/快/暂无分级。
- 删除“X 个关联渠道”摘要、旧 `<details>` 和覆盖统计的面板；主体右侧直接显示图 3 信息层级的短渠道卡，时间口径仍为最近 1 分钟，时间线固定约 10–16 段。
- 默认渠道与“全部站点”同站点卡片复用同一最终关联和主展示选择；点击短卡打开全部关联渠道弹框，支持内容区滚动、关闭图标、Escape、遮罩关闭、焦点进入和归还。
- loading、unsupported、error、无 Key 分组、无关联和近一分钟无数据使用同尺寸稳定状态，不打开空弹框；固定 `380×260` 与现有刷新、缓存、退避和跨站隔离保持不变。
