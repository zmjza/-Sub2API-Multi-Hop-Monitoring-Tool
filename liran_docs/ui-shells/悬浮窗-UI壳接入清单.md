...
已接入真实当前站、余额、默认 Key、倍率、今日 Token/消费、动态标题、真实状态行、缓存与更新时间；左右切站持久化当前站并跨窗口同步，扩大按钮隐藏悬浮窗并打开、聚焦主面板。四角位置已持久化，默认右上。悬浮窗保持已显示但 `alwaysOnTop:false`，浏览器等前台应用可覆盖；macOS 台前调度/多 Space 可见且不覆盖全屏应用。开发与 macOS 打包应用验证已通过；物理多显示器因当前环境不适用。
