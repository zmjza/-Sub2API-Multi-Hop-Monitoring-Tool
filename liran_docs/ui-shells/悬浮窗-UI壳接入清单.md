...
已接入真实当前站、余额、默认 Key、倍率、今日 Token/消费、动态标题、真实状态行、缓存与更新时间；左右切站持久化当前站并跨窗口同步，扩大按钮打开并聚焦主面板。四角位置已持久化，默认右上。悬浮窗使用普通桌面层级且 `alwaysOnTop:false`。开发与 macOS 打包应用 E2E、四角持久化和页面几何检查已通过；物理多显示器因当前环境不适用。
