...
## 2026-09-20 REQ-260920 待实现边界

- V2 渠道比例需兼容 0..1 和 1..100，统一为 0..100 后再判断；缺失、非有限、越界不得进入健康阈值。
- `coverage.data_through`、bootstrap、partial、no-sample、stale、鉴权、临时失败和 unsupported 保持不同语义；摘要和 matrix 必须来自同一代快照。
- 当前 auto 路径事实为 V1 优先，明确 prefer=v2 时直读 V2。协议选择是否调整只依据真实站点对照，不按历史冲突文案改代码。
- V2 红色误判根因尚未确认；timezone 只是待对照字段，禁止提前定因或添加站点特判。

## 2026-09-15 REQ-260915 探测顺序不改

- 本任务不改能力探测顺序。渠道状态继续 V1 优先，仅明确不可用才回退 V2；429/5xx/超时保留缓存并下周期重探测。
- 模块 06 旧文若写 V2 先探测，以正式 PRD 和本条为准，开发目标不得跟着旧文改顺序。
