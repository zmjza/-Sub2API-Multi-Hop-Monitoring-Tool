...
## 2026-09-20 REQ-260920 数据与通知边界（待实现）

- Renderer 不获得数据库、系统通知或完整上游响应能力；V2 对照只保存脱敏参数和字段结论。
- 缓存率日志最多记录 siteId、groupId/规范化身份、状态和错误类别，不记录 API Key、账号、完整 URL、请求正文或 Token 明细。
- 设置继续走严格共享 schema 和现有 IPC 白名单，不开放任意通知内容或定时器控制。
