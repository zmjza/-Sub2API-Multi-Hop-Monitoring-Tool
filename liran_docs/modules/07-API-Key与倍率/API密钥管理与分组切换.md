...
## 2026-09-20 3.3.0 高度与分组菜单

- 页面内容区填满主窗口剩余高度；左侧渠道卡列表独立滚动，Key 数量不再决定页面高度。
- 分组菜单通过 portal 挂载到 body，使用 fixed 视口坐标并按上下空间翻转，避免表格 overflow 裁切。

## 2026-09-15 REQ-260915 表格与名称列

- 保留名称、API 密钥、分组、平台、有效倍率、当前并发、消费、状态、创建时间九列；空间不足只表内横滚。
- 删除分组下拉下方重复分组名；名称列水平垂直居中。复制、脱敏、分组写入回滚语义不变。
...
在主导航新增普通用户 API 密钥页面，完整分页展示当前用户 Key 并允许单 Key 切换到有权分组。自 1.4.3 起完整 Key 可在管理页 Renderer 会话内存中短暂展示，复制由主进程完成；不得写入 SQLite、日志、缓存、CSV、截图或测试夹具。首版不解绑、不批量、不创建、不删除、不修改其他属性。
...
- AK-11 的历史“禁止显示完整 Key 或复制按钮”已被用户确认的 1.4.3 优化覆盖：页面显示完整 Key，点击复制走主进程 IPC；安全边界改为不落盘、不日志、不缓存、不截图、不写测试夹具。
- 新增 API Key 页面列视觉、平台图标、倍率图标、分组 option 预览和消费合并验收；悬浮窗今日 Token/消费改用当前有效 Key 统计。
