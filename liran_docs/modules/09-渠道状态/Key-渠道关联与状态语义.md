...
## 2026-09-20 REQ-260920 快照语义（待实现）

- V2 分组身份优先稳定 groupId，缺少 ID 时才使用规范化名称；显示名称不作为跨站共享键。
- 渠道健康、覆盖完整度、同步新鲜度继续正交；缓存率告警按站点+分组独立，不改变现有手动 Key-渠道关联优先级。

## 2026-08-07 1.9.2 悬浮窗最近 12 次规则（已实现并通过真机）

- 展示数据只来自 `GET /channel-monitors` 的 `timeline`；过滤无效 `checkedAt`，按时间升序取最后 12 条，不持久化或拼接第二套本地历史。
- 每格是一条真实检查，左旧右新。少于 12 条时只在左侧补空槽；`normal/degraded/failed/unknown` 分别为绿/黄/红/灰，空槽使用独立浅灰语义。
- 可用率只以截取后的真实记录为分母；`failed/error/down/unavailable` 归一化为明确失败，其余既有状态语义不擅自扩大。零记录不计算百分比。
- 本规则只用于悬浮窗主卡和关联渠道弹框。下面 1.9.1 的最近一分钟规则继续用于倍率稳定性等既有流程，禁止静默替换共享业务口径。

## 2026-08-07 1.9.1 悬浮窗呈现规则（已完成）

- 全部站点与悬浮窗必须调用同一纯函数取得最终一对多关联和主展示渠道，禁止各自取数组项或按健康状态另行排序。
- 最近一分钟视图只接受 `checkedAt >= now - 60_000` 的有效时间；`failed/error/down/unavailable` 为明确失败，其余状态计为可用，零检查点显示“近 1 分钟无数据”。
- 紧凑时间线从有效一分钟检查点中按时间排序并截取固定短段；详情弹框展示全部关联渠道，主展示渠道仅做克制选中标识。
...
- 渠道状态页面不再提供独立的 Key 分组批量关联面板。
- 关联入口位于全部站点的“查看渠道状态”弹层；每个渠道卡片右上角独立显示“关联”或“已关联”。
- 点击按钮只切换当前渠道的 `channelIds[]` 成员，不改变详情渠道；保存按 `siteId + groupId` 复用现有 IPC，失败时回滚本地状态。
- 一个分组可以同时关联多个渠道；取消最后一个渠道时复用现有空数组清除语义，自动关系解析、缓存和总览摘要保持不变。
