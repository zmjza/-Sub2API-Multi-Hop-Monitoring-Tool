...
需求：RQ-26、RQ-27
状态：已完成；结构化关联、唯一高置信回退、状态语义和渠道去折算 UI 通过

## 关系与状态模型

关系先按 Key.groupId 定位 available channel，之后再把 channel 映射到 monitor。`group_id` 与 `monitor.id` 属于不同实体，禁止比较。健康、同步、新鲜度和关联状态正交：remote failed/error 才是“异常”，请求失败是“刷新失败”，旧数据是“数据过期”，无关系是“未关联”。
