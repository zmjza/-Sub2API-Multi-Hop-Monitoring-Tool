...
| Radar 嵌入站点 | 无应用业务 API；Renderer 发送固定目标枚举，由主进程创建 `WebContentsView` | 独立内存 partition；不继承项目 preload、凭据或 IPC                                                                             | 两个固定 HTTPS 网页入口及生命周期状态               | 1.8.1 macOS 真实 Electron 流程已验证             |
...
- Radar 不再使用 `current.json`。两个远程站点只作为固定 `WebContentsView` 顶层入口；目标枚举、精确 origin 白名单、新窗口拒绝和视图生命周期由主进程负责，不进入 sub2api 认证、Token、数据库或 CSV 流程。
