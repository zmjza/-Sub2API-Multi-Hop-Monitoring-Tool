...
Radar 是正式主导航中的入口选择页，不再读取 `current.json` 或构建本地模型数据。Renderer 只发送固定的 `codex`/`distributed` 目标枚举；主进程将枚举映射到 `https://codexradar.com/` 或 `https://deng.codexradar.com/`，在当前主 `BrowserWindow` 的内容区挂载独立 `WebContentsView`。远程视图使用 sandbox、contextIsolation、nodeIntegration=false 和独立内存 partition，不注入项目 preload，不接触站点凭据、IPC、SQLite 或本地文件。

远程视图的顶层导航只接受两个精确 HTTPS origin，新窗口和额外 webview 均拒绝；主窗口 resize、关闭、应用退出、远程视图销毁和 `Esc` 都由主进程清理。应用自己的 toolbar 与右上角关闭按钮保留在 WebContentsView 上方，加载失败时移除远程视图并由 Renderer 显示可恢复错误状态。主应用 CSP 不再放行 Radar 外部连接，远程站点使用自身会话和策略。
