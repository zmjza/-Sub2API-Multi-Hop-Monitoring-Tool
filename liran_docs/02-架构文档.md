...
## 2026-08-14 2.2.0 常用网站架构

- 数据链路：FavoriteWebsitesPage -> preload favoriteWebsites -> IPC 主进程 FavoriteWebsitesManager -> AppDatabase settings；URL 与规则判定集中在 electron/shared/favorite-websites.ts。
- 每个网站使用 persist:favorite-website-{id} 独立分区 WebContentsView；内嵌页面保持 sandbox、contextIsolation、nodeIntegration=false、webSecurity=true，禁止 webview 和 window.open。
- 顶层导航同时监听 will-navigate 与 will-redirect，使用 isUrlAllowedByPolicy 按当前地址类型开关和自定义规则判定；同 origin 路径/查询/hash 不额外拦截，跨 origin 和 HTTP/HTTPS 切换重新校验。
- 永久禁止规则在 normalizeFavoriteWebsiteUrl 和 parseCustomAllowRule 两层拒绝，用户规则只能扩大 HTTP/HTTPS 范围，不能解除 file、javascript、data、devtools 或用户名密码地址。
- 删除网站和编辑导致 origin 变化时，通过 session.fromPartition 清理 cookies、localstorage、indexdb、cachestorage、serviceworkers 和 shadercache；Renderer 不接触 Cookie、Token 或完整页面存储。
...
2.5.0 起的分工：站点卡片、渠道状态弹层与悬浮窗的最终展示来源固定为用户手动保存的 `channelIds`。总览/悬浮窗通过 `resolveChannelPresentation` 的“仅手动关联”模式得到 `matched(manual)/unmatched`，没有手动关联即显示“暂未关联渠道”，不再把自动关系当作最终展示。倍率推荐内部继续使用“手动优先、自动候选兜底”的带状态候选池，自动候选只参与推荐评分，不成为展示来源；`/channels/available` 关系、分组名、平台、模型与 monitor ID 均不得重新成为展示联接受源。
...
## 2026-08-10 2.1.2 Sub2API 快捷入口内置模板架构增量

- 新增 `electron/shared/sub2api-menu-template.ts`，集中声明 `SUB2API_STANDARD_MENUS`、分组标签、路径匹配和旧快捷入口归一化；Renderer 直接本地渲染，不再经过主进程菜单发现 IPC。
- 删除主进程临时隐藏 `WebContentsView`、`discover-menus`、`list-menus`、菜单提取脚本和对应 preload 暴露；实际内嵌网页仍使用 `WebContentsView`，其安全参数、同源白名单、登录态和 partition 生命周期不变。
- `sub2api-servers:menus:<serverId>` 旧缓存不再读取；`clearSub2ApiMenus` 保留为删除服务器、清除会话和 origin 变更时的按服务器清理。
- 编辑弹窗状态只由本地模板、当前草稿快捷入口和搜索词决定；保存时对旧入口执行 `normalizeSub2ApiShortcutForTemplate`，匹配项补齐标准 `menuId/icon/label/path`，未匹配项原样保留。
- 卡片展示时先按模板顺序排标准入口，历史入口保持在后面；快捷入口点击继续走 `openShortcut -> sub2apiShortcutUrl -> open(id, url)`。

# 2.5.2 刷新与跨窗口数据流

主进程 `scheduleRefreshLoops` 负责站点 30–60 秒随机刷新和渠道 10–20 秒随机刷新。`SiteService` 保存渠道缓存、`fetchedAt` 与 stale 错误信息，通过 `channels:changed` 广播给所有 Renderer；倍率上下文在渠道批次后刷新并通过 `rates:changed` 广播。Renderer 只消费缓存事件，页面级渠道后台定时器已移除。

# 2.6.0 展示与切换修订

- `App` 维护按 `siteId` 索引的渠道广播快照，`OverviewPage` 收到快照后同步 loader、卡片渠道列表、关系、状态和 stale 相关 ViewModel，避免只更新隐藏缓存而不触发卡片重渲染。
- 中转站 `UsagePage` 使用独立 `缓存率` 列；`OpenCodexUsagePage` 不显示缓存率，两个模式仍共用缓存率计算函数。
- Sub2API 服务器切换允许从 `open/error` 状态进入目标服务器；主进程加载目标页面成功后提交替换，失败时移除临时视图并恢复旧视图。
