...
- 悬浮窗固定为 `380 x 260`，以工作区 12px 边距停靠左上、右上、左下或右下，默认右上并写入 SQLite。所有平台保持 `alwaysOnTop=false`，切换应用不调用 `hide()` 或 `minimize()`，因此窗口继续存在但可被浏览器等前台应用覆盖。macOS 额外使用 `setVisibleOnAllWorkspaces(true, { visibleOnFullScreen: false })` 适配台前调度和 Space；Windows 不启用跨工作区或全屏覆盖。
